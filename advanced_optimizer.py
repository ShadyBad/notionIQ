...
class ContentPrioritizer:
    ...
    def __init__(self, config: OptimizationConfig):
        ...
        self.priority_keywords = {
            ContentPriority.CRITICAL: [
                ...
                'deadline', 'overdue', 'today', 'now'
            ],
            ContentPriority.HIGH: [
                'important', 'priority', 'meeting', 'todo', 'task',
                'action', 'review', 'feedback', 'this week'
            ],
            ContentPriority.MEDIUM: [
                'project', 'plan', 'idea', 'note', 'document',
                'draft', 'proposal', 'research', 'wiki', 'dashboard',
                'hub', 'system', 'analysis', 'management'
            ],
            ContentPriority.LOW: [
                'archive', 'old', 'backup', 'copy', 'untitled', 'empty',
                'test page', 'sample page', 'template page'
            ]
        }
    ...
    def calculate_priority(self, page_content: Dict[str, Any]) -> ContentPriority:
        ...
        content = (page_content.get('title', '') + ' ' + 
                  page_content.get('content', '')).lower()
    ...
    def should_analyze(self, priority: ContentPriority, sampling_enabled: bool = True) -> bool:
...
class ProgressiveAnalyzer:
    ...
    def should_continue_analysis(self, pages_analyzed: int, total_pages: int) -> bool:
        ...
        if self.analysis_history:
            ...
            classifications = [a.get('classification', {}).get('primary_type') 
            ...
            if classifications:
                ...
                if most_common[1] / len(classifications) > 0.8:
                    logger.info(f"Pattern detected: {most_common[0]} dominates recent analyses")
    ...
    def predict_classification(self, content: Dict[str, Any]) -> Optional[Dict[str, Any]]:
...
class CostTracker:
    ...
    def can_make_request(self, estimated_cost: float) -> bool:
        ...
        if self.hourly_costs[current_hour] + estimated_cost > hourly_limit:
            logger.warning(f"Hourly rate limit reached: ${self.hourly_costs[current_hour]:.2f}")
            return False
    ...
    def record_cost(self, actual_cost: float):
        ...
        logger.info(f"Cost recorded: ${actual_cost:.4f} (Daily total: ${self.daily_costs[today]:.2f})")
...
class AdvancedOptimizer:
    ...
    def __init__(self, settings, config: Optional[OptimizationConfig] = None):
        ...
        logger.info(f"Advanced Optimizer initialized with daily budget: ${self.config.max_daily_cost}")
    ...
    def optimize_batch(self, pages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        if max_pages < len(prioritized):
            logger.warning(f"Budget allows only {max_pages} pages, have {len(prioritized)}")
        ...
        logger.info(f"Optimization complete: {len(optimized)}/{len(pages)} pages selected")
...
{{"type":"<task|project|note|idea|meeting|reference>","action":"<move|archive|keep|review>","confidence":0.0}}"""
    ...
    def should_stop_analysis(self, pages_analyzed: int, total_pages: int) -> bool:
        ...
        return not self.progressive.should_continue_analysis(pages_analyzed, total_pages)