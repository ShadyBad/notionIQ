...
class UniversalAIAnalyzer:
    ...
    def __init__(
        ...
        logger.info(
            f"AI Analyzer initialized with {self.ai_config['provider']} - {self.ai_config['model_info']['name']}"
        )
    ...
    def _configure_ai_provider(
        ...
        if preferred_provider:
            ...
            if preferred_provider:
                logger.info(
                    f"Using environment-specified provider: {preferred_provider}"
                )
    ...
    def _get_openai_response(self, prompt: str) -> str:
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                response_format=(
                    {"type": "json_object"}
                    if self.ai_config["model_info"]["supports_json"]
                    else None
                ),
            )
...
if __name__ == "__main__":
    ...
    print(
        f"   Cost: ${analyzer.ai_config['model_info']['cost_per_1m_input']}/1M input tokens"
    )
