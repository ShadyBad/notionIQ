...
class AIProviderManager:
    ...
    def select_provider(
        ...
        if not allow_multi_provider and len(self.detected_providers) > 1:
            logger.warning(f"⚠️ Multiple AI providers detected: {list(self.detected_providers.keys())}")
            ...
            logger.info(f"📌 Using single available provider: {selected_provider.value}")
        elif len(self.detected_providers) == 0:
            raise ValueError("No AI provider API keys found! Please configure at least one.")
        else:
    ...
    def _resolve_provider_conflict(self) -> AIProvider:
        ...
        if "last_selected_provider" in self.user_preferences:
            ...
            if last_provider in self.detected_providers:
                logger.info(f"📌 Using previously selected provider: {last_provider.value}")
        ...
        if env_preference:
            for provider in self.detected_providers:
                if provider.value.lower() == env_preference:
                    logger.info(f"📌 Using environment preferred provider: {provider.value}")
                    return provider
    ...
    def _select_best_model(
        ...
        if cost_priority:
            ...
            sorted_models = sorted(
                available_models,
                key=lambda m: -self.MODEL_INFO[m].quality_score
            )
        else:
            ...
            sorted_models = sorted(
                ...
                key=lambda m: (
                    ...
                    (self.MODEL_INFO[m].cost_per_1m_input + 
                     self.MODEL_INFO[m].cost_per_1m_output + 1)
                )
            )
        ...
        if not self.detected_providers:
            ...
            for provider, data in self.detected_providers.items():
                ...
                for model in data["available_models"]:
                    ...
                    print(f"    - {info.name}: ${cost:.2f}/1M tokens, Quality: {info.quality_score}/10")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"   Cost: ${config['model_info']['cost_per_1m_input']:.2f} input + ${config['model_info']['cost_per_1m_output']:.2f} output per 1M tokens")
