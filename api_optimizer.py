...
class TokenOptimizer:
    ...
    def __init__(self, optimization_level: OptimizationLevel = OptimizationLevel.MINIMAL):
    ...
    def optimize_prompt(self, prompt: str) -> str:
        ...
        if self.optimization_level == OptimizationLevel.MINIMAL:
            ...
            essential_lines.append('{"classification":{"primary_type":"<type>","confidence":0.0},"recommendations":{"primary_action":"<action>","suggested_database":"<db>"}}')
...
class SmartCache:
    ...
    def _generate_fingerprint(self, content: Dict[str, Any]) -> str:
        ...
        features = {
            ...
            'content_length': len(content.get('content', '')) // 100,  # Bucket by 100 chars
            'properties': sorted(content.get('properties', {}).keys()),
            'has_dates': bool(re.search(r'\d{4}-\d{2}-\d{2}', str(content))),
            'has_todos': 'todo' in str(content).lower() or 'task' in str(content).lower(),
                ...
                try:
                    ...
                    if datetime.now() - cached_data['timestamp'] < self.ttl:
                        similarity = self._calculate_similarity(
                            content,
                            cached_data.get('original_content', {})
                        )
    ...
    def _save_cache(self):
        ...
        with open(index_file, 'w') as f:
            json.dump(
                {k: str(v) for k, v in self.fingerprint_cache.items()},
                f
            )
...
class RequestBatcher:
    ...
    def process_batch_with_cache(self, batch: List[Dict[str, Any]], processor_func) -> List[Dict[str, Any]]:
...
class APIOptimizer:
    ...
    def optimize_page_content(self, page_content: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'content' in optimized:
            ...
            optimized['content'] = self.token_optimizer.optimize_content(
                optimized['content'],
                max_length=300 if self.optimization_level == OptimizationLevel.MINIMAL else 1000
            )
        ...
        if self.optimization_level == OptimizationLevel.MINIMAL:
            if 'properties' in optimized:
                ...
                optimized['properties'] = {
                    k: v for k, v in optimized['properties'].items()
    ...
    def check_cache_and_similarity(self, content: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def should_skip_page(self, page_content: Dict[str, Any]) -> bool:
        ...
        if self.optimization_level == OptimizationLevel.MINIMAL:
            ...
            empty_template_indicators = [
                '[placeholder]', '{{', 'lorem ipsum', 'sample text',
                'untitled page', 'new page'
            ]
            
            # Skip only if title AND content suggest it's empty template
            title_is_template = any(indicator in title_lower for indicator in ['untitled', 'new page', 'template page'])
            content_is_template = any(indicator in content_lower for indicator in empty_template_indicators)
