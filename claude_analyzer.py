...
from config import Settings, get_settings, CLASSIFICATION_CONFIG
from api_optimizer import APIOptimizer, OptimizationLevel, TokenOptimizer
from advanced_optimizer import AdvancedOptimizer, OptimizationConfig, ContentPriority
...
class ClaudeAnalyzer:
    ...
    def __init__(self, settings: Optional[Settings] = None, optimization_level: OptimizationLevel = OptimizationLevel.MINIMAL):
    ...
    def analyze_page(
        ...
        logger.info(f"API Call - Input: {input_tokens} tokens, Output: {output_tokens} tokens, Cost: ${self.api_optimizer.token_optimizer.calculate_cost(input_tokens, output_tokens):.4f}")
  ...
  "recommendations": {{"primary_action": "<move|archive|keep|review>", "suggested_database": null}}
        ...
        if self.provider == "anthropic":
            try:
                message = self.client.messages.create(
                    model=self.model,
                    max_tokens=100 if self.optimization_level == OptimizationLevel.MINIMAL else 500,
    ...
    def batch_analyze(
        ...
        if total < original_total:
            console.print(f"[yellow]Reduced pages by {original_total - total} through advanced optimization[/yellow]")
        ...
        for i, page in enumerate(pages, 1):
            ...
            if self.advanced_optimizer.should_stop_analysis(i, total):
                console.print(f"[yellow]Stopping early due to pattern detection (analyzed {i}/{total})[/yellow]")
                break
            
            # Check for predicted analysis
            if 'predicted_analysis' in page:
                results.append(page['predicted_analysis'])
                console.print(f"[dim]Using predicted classification for: {page.get('title', 'Untitled')}[/dim]")
                continue
        ...
        console.print(f"  Similar Pages Skipped: {usage_report['similar_pages_skipped']}")
