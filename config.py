...
class Settings(BaseSettings):
    ...
    output_dir: Path = Field(Path("output"), description="Directory for output files")
    data_dir: Path = Field(Path("data"), description="Directory for data files")

    # Processing Settings
    batch_size: int = Field(
        10, ge=1, le=100, description="Number of pages to process in batch"
    )
    max_content_length: int = Field(
        10000, ge=1000, le=50000, description="Maximum content length per page"
    )
    ...
    rate_limit_requests_per_second: float = Field(
        3.0, ge=0.1, le=10.0, description="API rate limit"
    )

    # Security Settings
    enable_encryption: bool = Field(False, description="Enable data encryption")
    ...
    enable_workspace_scan: bool = Field(
        True, description="Enable full workspace scanning"
    )
    enable_pattern_learning: bool = Field(
        True, description="Enable pattern learning from user actions"
    )
    enable_auto_organization: bool = Field(
        False, description="Enable automatic page organization"
    )
    enable_recommendations_page: bool = Field(
        True, description="Create/update recommendations page in Notion"
    )
    auto_execute: bool = Field(
        False, description="Automatically execute recommendations without confirmation"
    )

    # Analytics Settings
    enable_analytics: bool = Field(True, description="Enable analytics tracking")
    track_time_saved: bool = Field(True, description="Track time saved metrics")
    track_accuracy: bool = Field(True, description="Track classification accuracy")
    ...
    def validate_ai_keys(self):
        ...
        try:
            ...
            if self.notion_api_key and not SecurityValidator.validate_notion_api_key(
                self.notion_api_key
            ):
            ...
            if (
                ...
                and not SecurityValidator.validate_anthropic_api_key(
                    self.anthropic_api_key
                )
            ):
                raise ValueError("Invalid Anthropic API key format")

            if self.openai_api_key and not SecurityValidator.validate_openai_api_key(
                self.openai_api_key
            ):
...
CLASSIFICATION_CONFIG = {
    ...
    "contexts": ["work", "personal", "learning", "creative", "administrative"],
    "actions": [
        "move_to_database",
        "archive",
        "delete",
