...
class CostMonitor:
    ...
    def calculate_cost(self, input_tokens: int, output_tokens: int, model: str = "claude-3-opus") -> float:
    ...
    def record_usage(
        ...
        cost = self.calculate_cost(input_tokens, output_tokens, model) if not cached else 0.0
    ...
    def get_current_metrics(self) -> CostMetrics:
        ...
        if metrics.total_calls > 0:
            metrics.average_input_tokens = metrics.total_input_tokens / metrics.total_calls
            metrics.average_output_tokens = metrics.total_output_tokens / metrics.total_calls
        ...
        if metrics.cache_hits > 0:
            avg_cost = metrics.average_cost_per_call if metrics.average_cost_per_call > 0 else 0.02
    ...
    def get_budget_status(self) -> Dict[str, Dict]:
        ...
        status['hourly'] = {
            ...
            'percentage': (hourly_spent / self.budget_limits['hourly'] * 100) if self.budget_limits['hourly'] > 0 else 0
        }
        ...
        status['daily'] = {
            ...
            'percentage': (daily_spent / self.budget_limits['daily'] * 100) if self.budget_limits['daily'] > 0 else 0
        }
        ...
        status['weekly'] = {
            ...
            'percentage': (weekly_spent / self.budget_limits['weekly'] * 100) if self.budget_limits['weekly'] > 0 else 0
        }
        ...
        status['monthly'] = {
            ...
            'percentage': (monthly_spent / self.budget_limits['monthly'] * 100) if self.budget_limits['monthly'] > 0 else 0
        }
    ...
    def display_dashboard(self):
        ...
        metrics_table.add_row("Avg Output Tokens", f"{metrics.average_output_tokens:.0f}")
        metrics_table.add_row("Avg Cost per Call", f"${metrics.average_cost_per_call:.4f}")
        metrics_table.add_row("Cache Hit Rate", f"{(metrics.cache_hits / (metrics.total_calls + metrics.cache_hits) * 100) if metrics.total_calls > 0 else 0:.1f}%")
        ...
        for period, status in budget_status.items():
            color = "red" if status['percentage'] > 80 else "yellow" if status['percentage'] > 50 else "green"
    ...
    def generate_report(self) -> Dict:
        ...
        # Save report
        report_file = self.data_dir / f"cost_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def _generate_recommendations(self, metrics: CostMetrics, budget_status: Dict) -> List[str]:
    ...
    def _check_budget_alerts(self):
        ...
        for period, info in status.items():
            if info['percentage'] > 90:
                logger.warning(f"Budget Alert: {period} spending at {info['percentage']:.1f}% of limit!")
            elif info['percentage'] > 75:
                logger.info(f"Budget Warning: {period} spending at {info['percentage']:.1f}% of limit")
