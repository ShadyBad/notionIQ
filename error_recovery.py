...
class CircuitBreaker:
    ...
    def call(self, func: Callable[..., T], *args, **kwargs) -> T:
        ...
        if self.state == CircuitState.HALF_OPEN:
            if self.half_open_requests >= self.config.half_open_requests:
                raise CircuitOpenError(f"Circuit breaker '{self.name}' is testing recovery")
    ...
    def _on_failure(self):
        ...
        if self.failure_count >= self.config.failure_threshold:
            ...
            logger.warning(f"Circuit breaker '{self.name}' is now OPEN after {self.failure_count} failures")
...
class ErrorRecoveryManager:
    ...
    def with_recovery(
        ...
        def wrapper(*args, **kwargs) -> T:
            ...
            if circuit_name:
                ...
                try:
                    return breaker.call(self._execute_with_retry, func, recovery_func, max_retries, *args, **kwargs)
                except CircuitOpenError:
                    if recovery_func:
                        logger.warning(f"Circuit open for {circuit_name}, using recovery function")
                        return recovery_func(*args, **kwargs)
                    raise
            else:
                return self._execute_with_retry(func, recovery_func, max_retries, *args, **kwargs)
        ...
        for retry_count in range(max_retries + 1):
            try:
                ...
                if retry_count < max_retries:
                    ...
                    logger.warning(f"Retry {retry_count + 1}/{max_retries} after {delay:.2f}s delay: {e}")
                    time.sleep(delay)
                else:
                    # Max retries reached
                    if recovery_func:
                        ...
                        try:
                            ...
                            logger.error(f"Recovery function also failed: {recovery_error}")
    ...
    def _classify_error_severity(self, error: Exception) -> ErrorSeverity:
        ...
        if any(term in error_msg for term in ['api key', 'authentication', 'unauthorized']):
    ...
    def _add_to_dead_letter_queue(self, func: Callable, args: tuple, kwargs: dict, error: Exception):
...
def with_recovery(
    ...
    def decorator(func):
        return error_recovery.with_recovery(func, recovery_func, circuit_name, max_retries)
    return decorator
