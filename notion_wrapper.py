...
class NotionAdvancedClient:
    ...
    def get_database(self, database_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(
                f"Retrieved database: {database.get('title', [{}])[0].get('plain_text', 'Untitled')}"
            )
            return database
        ...
        try:
            ...
            if self.settings.enable_caching:
                ...
                self.cache_expiry[cache_key] = (
                    time.time() + self.settings.cache_ttl.total_seconds()
                )
    ...
    def _analyze_database_properties(self, database: Dict) -> Dict[str, Any]:
        ...
        for prop_name, prop_config in properties.items():
            ...
            if prop_config.get("type") == "relation":
                analyzed[prop_name]["database_id"] = prop_config.get(
                    "relation", {}
                ).get("database_id")
            elif prop_config.get("type") == "rollup":
                analyzed[prop_name]["relation_property"] = prop_config.get(
                    "rollup", {}
                ).get("relation_property_name")

        return analyzed

    def _detect_database_relationships(
