...
"""
    ...
    try:
        ...
        print(f"✅ Cost calculation works: ${cost:.4f} for 1000 input + 500 output tokens")
    except Exception as e:
...
def test_optimization_levels():
    ...
    test_content = "This is a comprehensive test of the content optimization system. " * 20
    
    for level in [OptimizationLevel.MINIMAL, OptimizationLevel.BALANCED, OptimizationLevel.FULL]:
        optimizer = TokenOptimizer(level)
        optimized = optimizer.optimize_content(test_content, max_length=500 if level == OptimizationLevel.MINIMAL else 2000)
