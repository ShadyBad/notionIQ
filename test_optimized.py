...
async def test_with_cost_tracking():
    ...
    try:
        ...
        console.print(
            f"  • Optimization: [cyan]MINIMAL[/cyan] (aggressive cost savings)"
        )
        ...
        console.print(
            f"  • Daily Spent: [yellow]${budget_status['daily']['spent']:.2f}[/yellow]"
        )
        console.print(
            f"  • Daily Remaining: [green]${budget_status['daily']['remaining']:.2f}[/green]"
        )
        ...
        try:
            test_pages = organizer.notion.get_database_pages(
                settings.notion_inbox_database_id, limit=1
            )
            if test_pages:
        ...
        console.print(
            "[dim]This will use minimal tokens and aggressive caching[/dim]\n"
        )
        ...
        if organizer.cost_monitor:
            ...
            console.print(
                f"  • API Calls Made: [cyan]{final_metrics.total_calls}[/cyan]"
            )
            console.print(
                f"  • Total Tokens Used: [cyan]{final_metrics.total_input_tokens + final_metrics.total_output_tokens:,}[/cyan]"
            )
            console.print(
                f"  • Test Cost: [bold green]${final_metrics.total_cost:.4f}[/bold green]"
            )
            console.print(f"  • Cache Hits: [cyan]{final_metrics.cache_hits}[/cyan]")
            console.print(
                f"  • Cost Saved by Caching: [green]${final_metrics.cost_saved:.4f}[/green]"
            )
            console.print(
                f"  • Daily Budget Used: [yellow]{final_budget['daily']['percentage']:.1f}%[/yellow]"
            )
        ...
        if hasattr(organizer.ai_analyzer, "api_optimizer"):
            ...
            console.print(
                f"  • Duplicates Skipped: [cyan]{usage_report['duplicates_skipped']}[/cyan]"
            )
            console.print(
                f"  • Similar Pages Skipped: [cyan]{usage_report['similar_pages_skipped']}[/cyan]"
            )
            console.print(
                f"  • Tokens Saved: [cyan]{usage_report['tokens_saved']:,}[/cyan]"
            )
        ...
        if report and organizer.cost_monitor:
            ...
            if pages_analyzed > 0:
                ...
                console.print(
                    f"  • 1000 Pages: [cyan]${estimated_100_pages * 10:.2f}[/cyan]"
                )

        console.print("\n[bold green]✅ Test completed successfully![/bold green]")
        console.print(
            "[dim]Ready to run full analysis with optimizations enabled[/dim]"
        )
...
if __name__ == "__main__":
    ...
    console.print(
        "[dim]This test will process only 3 pages with aggressive optimization[/dim]\n"
    )
    ...
    console.print(
        "[cyan]python notion_organizer.py --optimization minimal --batch-size 10[/cyan]"
    )
    console.print("\nOr for more complete analysis:")
    console.print(
        "[cyan]python notion_organizer.py --optimization balanced --daily-budget 10[/cyan]"
    )
