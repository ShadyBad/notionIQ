...
from config import HEALTH_METRICS_CONFIG, Settings, get_settings
...
class WorkspaceAnalyzer:
    ...
    def __init__(
        self, notion_client: NotionAdvancedClient, settings: Optional[Settings] = None
    ):
        ...
        for db_id, db_data in databases.items():
            ...
            if page_count == 0:
                ...
                analysis["large_databases"].append(
                    {"title": db_title, "count": page_count}
                )
            ...
            analysis["database_types"][db_type] = (
                analysis["database_types"].get(db_type, 0) + 1
            )
    ...
    async def _analyze_content_patterns(self):
        ...
        inbox_pages = self.notion.get_database_pages(
            self.settings.notion_inbox_database_id, limit=50  # Sample size
        )
        ...
        if patterns["content_lengths"]:
            patterns["avg_content_length"] = statistics.mean(
                patterns["content_lengths"]
            )
            patterns["median_content_length"] = statistics.median(
                patterns["content_lengths"]
            )
    ...
    async def _calculate_health_metrics(self):
        ...
        # Organization score
        org_factors = {
            "has_multiple_databases": len(
                self.notion.workspace_structure.get("databases", {})
            )
            > 1,
            "uses_properties": self._check_property_usage(),
            "has_relationships": self.patterns.get("relationship_analysis", {}).get(
                "total_relationships", 0
            )
            > 0,
            "consistent_naming": self._check_naming_consistency(),
            "no_empty_databases": len(
                self.patterns.get("database_analysis", {}).get("empty_databases", [])
            )
            == 0,
        }

        org_weights = HEALTH_METRICS_CONFIG["organization_score"]["weights"]
        metrics["organization_score"] = self._calculate_weighted_score(
            org_factors, org_weights
        )
        ...
        efficiency_factors = {
            ...
            "uses_title_patterns": len(content_patterns.get("top_title_patterns", []))
            > 0,
            "properties_utilized": self._check_property_utilization(),
        }

        metrics["efficiency_score"] = (
            sum(efficiency_factors.values()) / len(efficiency_factors) * 100
        )
        ...
        metrics["completeness_score"] = (
            sum(completeness_factors.values()) / len(completeness_factors) * 100
        )
        ...
        if total_dbs > 0:
            metrics["relationship_score"] = (
                (total_dbs - orphan_count) / total_dbs
            ) * 100
    ...
    def _calculate_weighted_score(
        self, factors: Dict[str, bool], weights: Dict[str, float]
    ) -> float:
    ...
    def _check_property_usage(self) -> bool:
        ...
        property_usage = self.patterns.get("database_analysis", {}).get(
            "property_usage", {}
        )
    ...
    def _check_naming_consistency(self) -> bool:
        ...
        title_patterns = self.patterns.get("content_patterns", {}).get(
            "top_title_patterns", []
        )
    ...
    def _check_property_utilization(self) -> bool:
        ...
        property_stats = self.patterns.get("content_patterns", {}).get(
            "property_statistics", {}
        )
        if property_stats:
    ...
    async def _generate_insights(self):
        ...
        empty_dbs = self.patterns.get("database_analysis", {}).get(
            "empty_databases", []
        )
        ...
        orphan_dbs = self.patterns.get("relationship_analysis", {}).get(
            "orphan_databases", []
        )
        if orphan_dbs:
            recommendations.append(
        ...
        large_dbs = self.patterns.get("database_analysis", {}).get(
            "large_databases", []
        )
    ...
    def _display_analysis_summary(self):
        ...
        metrics_display = [
            (
                ...
                (
                    "✅"
                    if len(self.notion.workspace_structure.get("databases", {})) > 3
                    else "⚠️"
                ),
            ),
            (
