...
class WorkspaceScanner:
    ...
    async def scan_entire_workspace(
        self,
        ...
        if target_databases:
            ...
            console.print(
                f"[yellow]Targeting {len(databases)} specific databases[/yellow]"
            )
        else:
            databases = {
                k: v
        ...
        with Progress(
            ...
            task = progress.add_task("Analyzing databases...", total=len(databases))
    ...
    async def _analyze_database_pages(
        self, db_id: str, db_name: str, pages: List[Dict]
    ) -> Dict:
    ...
    def _summarize_database_content(self, classifications: List[Dict]) -> Dict:
        ...
        return {
            ...
            "needs_review": sum(
                1 for c in classifications if c.get("confidence", 0) < 0.7
            ),
        }
    ...
    def _create_workspace_report(self, total_pages: int, insights: Dict) -> Dict:
        ...
        report = {
            ...
            "summary": {
                ...
                "empty_databases": sum(
                    1
                    for d in self.database_summaries.values()
    ...
    def _display_summary(self, total_pages: int, insights: Dict):
        ...
        table.add_row(
            "Document Types Found", str(len(insights.get("content_distribution", {})))
        )
        table.add_row(
...
async def main():
    ...
    parser.add_argument(
        "--skip-databases", nargs="+", default=[], help="Database names to skip"
    )
    parser.add_argument(
        "--target-databases",
        nargs="+",
